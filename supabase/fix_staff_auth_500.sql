...
-- Repair existing accounts before replacing the staff creation function.
UPDATE auth.users
SET
...
-- Ensure email identities exist for staff accounts created through SQL.
...
BEGIN
  ...
  -- Keep Auth text token fields as empty strings; NULL can make /auth/v1/token return 500.
  ...
  -- Mandatory: create identity record so the user can login with email/password.
  INSERT INTO auth.identities (
    id,
    user_id,
    identity_data,
  ...
  -- The Supabase trigger 'on_auth_user_created' fires above and inserts public.users.
  UPDATE public.users
